...
AC_DEFUN(AM_PATH_AOPP,
[
AC_MSG_CHECKING(for libaopp)
AC_PATH_PROG(AOPP_CONFIG, aopp-config, no)
if test "AOPP_CONFIG" = "no"
then
  AC_MSG_RESULT(not found)
else
  AOPP_CXXFLAGS=`$AOPP_CONFIG --flags`
  AOPP_LIBS=`$AOPP_CONFIG --libs`
  AC_SUBST(AOPP_CXXFLAGS)
  AC_SUBST(AOPP_LIBS)
fi
])
